...
public partial struct ShooterJob : IJobEntity
{
    ...
    void Execute(
        ...
        in AttackDamageData damageData,
        in AttackSpeedData attackSpeedData,
        in ProjectileCountData countData,
        in ProjectileDurationData durationData,
        in ProjectileSpeedData speedData)
    {
        if (!canfireData.CanFire) return;

        shooterData.ElapsedTime += DeltaTime * attackSpeedData.AttackSpeed;
