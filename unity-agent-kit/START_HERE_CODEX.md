...
먼저 `unity-agent-kit/FOR_CODEX.md`, `unity-agent-kit/HARNESS_INSTALL_SPEC.md`, `unity-agent-kit/templates/**`를 읽고, 현재 프로젝트 구조를 조사한 뒤, 이 프로젝트에 맞는 운영 문서, 검증 흐름, 에이전트 설정을 설치하거나 갱신해줘.
...
- Codex로 시작하더라도 Claude Code용 `.claude/**`, `.mcp.json`까지 같이 설치
...
- 구조 이해용으로 `graphify`를 붙일 계획이 있으면 `docs/Graphify.md`도 같이 설치하되, 기존 하네스 문서/훅을 덮어쓰는 `graphify codex install`/`graphify claude install`은 기본값으로 쓰지 마
...
`unity-agent-kit/FOR_CODEX.md`와 `unity-agent-kit/HARNESS_INSTALL_SPEC.md`를 먼저 읽고, 기존 AGENTS.md, README.md, CLAUDE.md, docs, tools, Assets/Editor, Assets/99_Tests를 비교해서 필요한 부분만 수정해줘.

기존 사용자 작업은 되돌리지 말고, 문서와 검증 흐름이 실제 프로젝트 상태와 어긋나는 부분만 맞춰줘.
Codex와 Claude Code를 같이 쓰는 handoff 구조가 빠져 있으면 그것도 보강해줘.
Codex로 시작해도 Claude Code용 프로젝트 로컬 설정까지 같이 맞춰줘.
