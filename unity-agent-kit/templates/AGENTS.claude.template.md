# {{PROJECT_NAME}} 에이전트 가이드 — Claude Code
...
<!-- CLAUDE.md가 세션 시작 시 자동 로드된다.
     폴더 맵, 검증, Guarded Assets는 CLAUDE.md에 있으므로 여기서 반복하지 않는다.
     아키텍처 규칙, Visual/Data Flow는 docs/project-context.md에 있다. -->
...
비자명한 수정 전에 `docs/AgentPromptTemplates.md`를 읽는다.
...
- RTK를 raw source 읽기, `Read/Grep/Glob`, `unity-cli`, `unity-prefab-parser-mcp`의 대체재로 취급하지 않는다
...
`docs/AgentHandoffs/**`를 Claude Code와 Codex 사이의 좁은 연결 통로로 사용한다.

사용자가 Codex 작업 확인을 요청하면:

- `docs/AgentHandoffs/pending/codex-to-claude/`를 먼저 확인한다
- placeholder 파일만 있으면 handoff 컨텍스트를 더 읽지 않는다
- 관련 pending 노트만 최신순으로 읽는다
- 이해한 노트는 `docs/AgentHandoffs/consumed/codex-to-claude/`로 이동한다
...
- `docs/AgentHandoffs/pending/claude-to-codex/`에 간단한 노트를 작성한다
...
→ `docs/project-context.md` (아키텍처 규칙 섹션) 참조

## 태스크 라우팅 힌트

프로젝트 고유 라우팅 예시를 작성한다:

- 순수 코드 변경 → compile + smoke
- UI/상태 동기화 → smoke + 수동 확인
- 데이터/설정 변경 → ScriptableObject 참조 검증
- 씬/프리팹 변경 → unity-cli + smoke
